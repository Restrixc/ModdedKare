...
public class InflatableDick : InflatableListener
{
	...
	public override void OnSizeChanged(float newSize)
	{
		...
		Vector3 squishedStartScale = startScale + Vector3.Lerp(-localForward * 0.5f, localForward * 0.5f, dickThicknessRatio) + Vector3.Lerp(localOthers * 0.25f, -localOthers * 0.25f, dickThicknessRatio);
		targetTransform.localScale = squishedStartScale * Mathf.Max(newSize, 0.05f);
		if (targetJiggleRoot != null)
		{
			targetJiggleRoot.localScale = startJiggleScale * Mathf.Max(newSize, 1f);
		}
