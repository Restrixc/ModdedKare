...
public class FluidStream : CatmullDeformer, IPunObservable, ISavable
{
	...
	[SerializeField]
	private VisualEffect splatter;

	[SerializeField]
	private AudioPack splatterSounds;

	[SerializeField]
	private AudioPack waterSpraySound;

	[SerializeField]
	private float waterRadiusCollision = 0.25f;
	...
	private void Awake()
	{
		photonView = GetComponentInParent<PhotonView>(includeInactive: true);
	}
	...
	private void Update()
	{
		...
		{
			rootBone.transform.localRotation *= Quaternion.AngleAxis((0f - Time.deltaTime) * 100f, localRootForward);
		}
		foreach (RendererSubMeshMask renderMask in GetTargetRenderers())
		{
	...
	private IEnumerator FireParticles()
	{
		while (firing)
		{
			Vector3 vel = rootBone.TransformDirection(localRootForward) * 0.1f;
			if (body != null)
			{
	...
	private bool RaycastStream(float startDistance, float endDistance, out RaycastHit hit, out float distance)
	{
		...
		for (int i = 0; i < points.Count - 1; i++)
		{
			...
			if (hits > 0)
			{
				...
				{
					if (raycastHits[j].point != Vector3.zero && raycastHits[j].distance < closestDist && distanceAcc + raycastHits[j].distance > startDistance && distanceAcc + raycastHits[j].distance < endDistance)
					{
	...
	private void OnSplash(RaycastHit hit, float percentageLoss)
	{
		...
		Material material = decalProjector;
		int colorID = ColorID;
		float? a = 1f / 51f;
		material.SetColor(colorID, color.With(null, null, null, a));
		Material material2 = decalProjectorSubtractive;
		int colorID2 = ColorID;
		a = 1f / 51f;
		material2.SetColor(colorID2, color.With(null, null, null, a));
		PaintDecal.RenderDecalInSphere(hit.point, 0.5f, midairContents.IsCleaningAgent() ? decalProjectorSubtractive : decalProjector, Quaternion.identity, GameManager.instance.decalHitMask);
		if (photonView.IsMine)
		{
			...
			if (cont != null)
			{
				cont.photonView.RPC("AddMixRPC", RpcTarget.All, midairContents.Spill(perVolume), container.photonView.ViewID);
			}
		}
	}
	...
	private IEnumerator Output()
	{
		...
		while (firing || Math.Abs(startClip - endClip) > 0.01f)
		{
			...
			if (container.volume > 0f && firing)
			{
				...
				splatter.SetVector4("Color", new Vector4(reagentColor.r, reagentColor.g, reagentColor.b, 1f));
			...
			if (RaycastStream(startClip * fluidLength, endClip * fluidLength, out var hit, out var distance))
			{
	...
	public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
	{
		...
		if (viewTarget != -1)
		{
			...
			if (view.TryGetComponent<GenericReagentContainer>(out var cont))
			{
				OnFire(cont);
			}
			else
