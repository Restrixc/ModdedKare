...
public class CommandSwap : Command
{
	...
	public override void Execute(StringBuilder output, Kobold k, string[] args)
	{
		...
		BrainSwapperMachine machine = UnityEngine.Object.FindObjectOfType<BrainSwapperMachine>();
		if (machine == null)
		{
		...
		Vector3 aimPosition = k.GetComponentInChildren<PlayerPossession>(includeInactive: true).GetEyePosition();
		Vector3 aimDir = k.GetComponentInChildren<PlayerPossession>(includeInactive: true).GetEyeDir();
		RaycastHit[] array = Physics.RaycastAll(aimPosition, aimDir, 5f);
		foreach (RaycastHit hit in array)
		{
			...
			machine.photonView.RPC("AssignKobolds", RpcTarget.All, k.photonView.ViewID, b.photonView.ViewID, bPlayer?.ActorNumber ?? (-1), aPlayer?.ActorNumber ?? (-1), b.GetComponent<MoneyHolder>().GetMoney(), k.GetComponent<MoneyHolder>().GetMoney());
