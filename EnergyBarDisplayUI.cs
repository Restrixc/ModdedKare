...
{
	[SerializeField]
	private float energyToPixel = 30f;
	...
	private IEnumerator EnergyLerpRoutine(float startWidth, float startBackgroundWidth)
	{
		running = true;
		...
		while (Time.time < startTime + duration)
		{
			...
			filledBar.sizeDelta = new Vector2(Mathf.Clamp(Mathf.LerpUnclamped(startWidth, targetWidth, bounceSample), 0f, float.MaxValue), filledBar.sizeDelta.y);
			barBackground.sizeDelta = new Vector2(Mathf.Clamp(Mathf.LerpUnclamped(startBackgroundWidth, targetBackgroundWidth, bounceSample), 0f, float.MaxValue), barBackground.sizeDelta.y);
			yield return null;
		}
