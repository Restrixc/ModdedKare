	...
	public override GameObject[] OnEquip(Kobold k, GameObject groundPrefab)
	{
		...
		foreach (PrefabAttachPointPair p in wearables)
		{
			...
			if (p.stacks)
			{
				...
				GameObject wearable2 = UnityEngine.Object.Instantiate(p.wearablePrefab, targetAttach);
				wearable2.transform.localPosition += offset;
				stuff[i++] = wearable2;
			}
			else
			{
				GameObject wearable = UnityEngine.Object.Instantiate(p.wearablePrefab, k.attachPoints[(int)p.attachPoint]);
				stuff[i++] = wearable;
			}
		}
		return stuff;
