...
public class InflatorPump : UsableMachine, IAnimationStationSet
{
	...
	private void Accumulate(float amount)
	{
		if (constructed)
		{
			...
			if (((double)amount > 0.1 || accumulation > 5f) && !spraying && container.volume > 0f)
			{
				pumpAnimator.SetBool(Pumping, value: true);
				spraying = true;
	...
	private IEnumerator SprayRoutine()
	{
		...
		if (base.photonView.IsMine)
		{
			...
			container.photonView.RPC("Spill", RpcTarget.Others, accumulation);
			Mozzarella mozzarella;
			if (pumper.TryGetPenetrable(out var penetrable))
			{
				GenericReagentContainer holeContainer = penetrable.GetComponentInParent<GenericReagentContainer>();
				if (holeContainer != null)
				{
					holeContainer.photonView.RPC("ForceMixRPC", RpcTarget.All, contents, container.photonView.ViewID);
				}
			}
			else if (MozzarellaPool.instance.TryInstantiate(out mozzarella))
			{
				...
				mozzarella.hitCallback += delegate(RaycastHit hit, Vector3 startPos, Vector3 dir, float length, float volume)
				{
					if (base.photonView.IsMine)
					{
						...
						if (componentInParent != null && componentInParent != container)
						{
					...
					PaintDecal.RenderDecalForCollider(hit.collider, alloc.IsCleaningAgent() ? cumCleanProjectorMaterial : cumSplatProjectorMaterial, hit.point - hit.normal * 0.1f, Quaternion.LookRotation(hit.normal, Vector3.up) * Quaternion.AngleAxis(Random.Range(-180f, 180f), Vector3.forward), Vector2.one * volume, length);
				};
				mozzarella.SetFollowPenetrator(pumper);
			}
		}
		accumulation = 0f;
		pumpAnimator.SetBool(Pumping, value: false);
		spraying = false;
		sloshSource.enabled = false;
	}
