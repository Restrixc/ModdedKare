...
public class DS4 : MonoBehaviour
{
	...
	public static Quaternion GetRotation(float scale = 1f)
	{
		...
		float z = (0f - ProcessRawData(pad.GetChildControl<ButtonControl>("gyro Z 18").ReadValue())) * scale;
		return Quaternion.Euler(x, y, z);
	}

	private static float ProcessRawData(float data)
	{
		return (data > 0.5f) ? (1f - data) : (0f - data);
	}
}
