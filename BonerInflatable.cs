...
public class BonerInflatable : InflatableListener
{
	...
	public override void OnEnable()
	{
		...
		using List<JiggleRigBuilder.JiggleRig>.Enumerator enumerator2 = rigBuilder.jiggleRigs.GetEnumerator();
		if (enumerator2.MoveNext())
		{
			JiggleRigBuilder.JiggleRig jiggleRig = enumerator2.Current;
			if (!(jiggleRig.jiggleSettings is JiggleSettingsBlend))
			{
				throw new UnityException("Jiggle rig needs a blended jigglesettings");
			}
			targetBlend = UnityEngine.Object.Instantiate((JiggleSettingsBlend)jiggleRig.jiggleSettings);
